...
| 2026-09-08 | Lucas | Exceção ao fallback gracioso (§4): um **404 / 410** do provider é definitivo — o adapter levanta `ArtworkGoneError` e o job apaga a referência (coluna `NULL` / campo removido do blob localizado via `json_remove`) em vez de re-tentar a cada tick; timeout e 5xx continuam mantendo a URL. |
| 2026-09-08 | Lucas | ADR-034: variantes redimensionadas (`<stem>.w<width><ext>`) derivadas do original espelhado — sob demanda em `GET ?w=` e pré-geradas pelo job logo após cada mirror; escrita do `LocalArtworkStorage` passa a ser atômica. |
| 2026-09-08 | Lucas | PR 3 (parcial): artes **localizadas** de Movie/Series mirroradas por (título, locale) — finder `json_each` + updater `json_set` campo a campo em `_artwork_helpers` (paths a partir de `LocalizedField`, locale validado e nunca canonicalizado), dois kinds novos no job com a mesma política LWW por campo. Fotos de elenco (`profile_path`) seguem como follow-up. Orçamento por tick passa a ser dividido em cotas iguais por kind (sobras rolam adiante): a divisão estrita em ordem deixava os kinds localizados sem orçamento enquanto uns poucos stills falhavam a cada tick. |
