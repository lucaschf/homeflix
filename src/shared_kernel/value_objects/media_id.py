"""Media external ID value objects.

Typed external IDs for media catalog entries:
- MovieId (mov_xxx)
- SeriesId (ser_xxx)
- SeasonId (ssn_xxx)
- EpisodeId (epi_xxx)

``MediaId`` lives in the shared_kernel (ADR-018) because media
identifiers cross bounded contexts: ``watch_progress``, ``collections``
and ``catalog_requests`` all reference catalog entries by these IDs.
"""
...
class MediaIdRuleCodes(StrEnum):
    """Rule codes for media ID parsing violations.

    String values are part of the API error contract and must match
    the codes historically emitted by the media module.
    """
...
def parse_media_id(value: str) -> MediaId:
    ...
    if not value or "_" not in value:
        raise DomainValidationException(
            ...
            message_code=MediaIdRuleCodes.INVALID_MEDIA_ID_FORMAT,
            object_type="MediaId",
        )
    ...
    match prefix:
        ...
        case _:
            raise DomainValidationException(
                ...
                message_code=MediaIdRuleCodes.UNKNOWN_MEDIA_PREFIX,
                object_type="MediaId",
            )
