"""Integration tests for the Collections ProfileViewingPolicyAdapter."""
...
from src.modules.collections.infrastructure.acl import ProfileViewingPolicyAdapter
...
class TestCollectionsProfileViewingPolicyAdapter:
    """The adapter exposes a follower's viewing policy to the Collections BC."""
    ...
    async def test_should_return_deny_all_policy_for_unknown_profile_id(
        ...
        # Deny-all is the safer default than raising — a followed list
        # must never reveal items to a profile that no longer exists.
        adapter = _make_adapter(session_factory)
