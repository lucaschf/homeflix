"""Tests for ChromaprintCorrelator.

The correlator is the pure cross-correlation algorithm; tests build
synthetic fingerprints (deterministic intro prefixes + random tails)
and assert the algorithm recovers the planted intro within tolerance.
No real fpcalc is required.
"""
...
_DEFAULT_TUNING = ChromaprintTuning()
...
@pytest.mark.unit
class TestChromaprintCorrelator:
    ...
    def test_detects_intro_with_varying_cold_open_lengths(self, shared_intro: list[int]) -> None:
        ...
        # correlator must align them via shift search.
    ...
    def test_tolerates_mild_per_hash_bit_noise(self, shared_intro: list[int]) -> None:
        ...
        # correlator still locks onto the segment.
        episodes = [
            _episode(
                intro_hashes=shared_intro,
                intro_offset_hashes=0,
    ...
    def test_drops_episode_below_pair_agreement_threshold(self, shared_intro: list[int]) -> None:
        ...
        correlator = ChromaprintCorrelator(min_pair_agreement=0.5)
        with_intro = [
            _episode(
                intro_hashes=shared_intro,
                intro_offset_hashes=0,
    ...
    def test_drops_short_matches_below_min_intro_seconds(self) -> None:
        ...
        # correlator should skip it entirely.
    ...
    def test_returns_empty_for_zero_duration_fingerprints(self) -> None:
        ...
        assert correlator.correlate(empty, _DEFAULT_TUNING) == {}
    ...
    def test_max_intro_seconds_clamps_long_matches(self) -> None:
        ...
        # at 5 s. The correlator finds the full match, then truncates the
        ...
        tuning = ChromaprintTuning(max_intro_seconds=5.0)
