"""End-to-end test scaffolding for the settings BC.

Mirrors the identity e2e setup: in-memory SQLite shared via
``StaticPool`` so seeds and HTTP-driven use cases see the same rows;
``ApplicationContainer`` wired by hand so the lifespan handler does
...
# Registering identity + settings models on ``Base.metadata`` so
# ``create_all`` discovers ``users`` / ``profiles`` / ``access_tokens``
# / ``app_settings``.
import src.modules.identity.infrastructure.persistence.models
import src.modules.settings.infrastructure.persistence.models  # noqa: F401
...
class SeededUser:
    """Convenience handle for the admin / member seed fixtures."""
