...
from src.modules.streaming.infrastructure.streaming.hardware_acceleration_probe import (
    HardwareAccelerationProbe,
)
from src.modules.streaming.infrastructure.streaming.hls_service import (
    HlsService,
    _append_endlist_atomic,
)
from src.modules.streaming.infrastructure.streaming.master_playlist_writer import (
    MasterPlaylistWriter,
)
from src.modules.streaming.infrastructure.streaming.media_probe_service import MediaProbeService
from src.modules.streaming.infrastructure.streaming.probe_cache_store import ProbeCacheStore
from src.modules.streaming.infrastructure.streaming.subtitle_pipeline import (
    _atomic_write_text,
    _ensure_vtt_timestamp_map,
    _shift_webvtt_to_bucket_local,
)
from src.modules.streaming.infrastructure.streaming.transcode_command_builder import (
...
class TestHlsServiceBuildAudioCmd:
    ...
    def _no_leading_gap_probe(self):  # type: ignore[no-untyped-def]
        ...
        with patch(
            "src.modules.streaming.infrastructure.streaming.transcode_command_builder._first_audio_pts",
            return_value=None,
        ):
            yield
...
class TestLeadingAudioGap:
    ...
    def test_first_pts_parses_ffprobe_output(self) -> None:
        ...
        with patch(
            "src.modules.streaming.infrastructure.streaming.transcode_command_builder.subprocess.run",
...
class TestEnsurePlaylistSoftwareFallback:
    ...
    async def test_falls_back_to_software_on_hw_failure(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch(
                "src.modules.streaming.infrastructure.streaming.hls_service.shutil.which",
                return_value="ffmpeg",
            ),
        ):
