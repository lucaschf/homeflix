"""End-to-end test scaffolding for the watch progress bounded context.

Mirrors the media e2e setup: in-memory SQLite shared via ``StaticPool``
so seeds and HTTP-driven use cases see the same rows;
``ApplicationContainer`` wired by hand so the lifespan handler does not
run. Unlike the media fixture, the seeded profile carries a library ACL:
the production ``ProfileViewingPolicyAdapter`` reads it, and an empty
ACL would deny everything.
"""
...
# Registering identity, media and watch progress models on
