"""Integration tests for the Watch Progress ProfileViewingPolicyAdapter."""
...
from src.modules.watch_progress.infrastructure.acl import ProfileViewingPolicyAdapter
...
class TestWatchProgressProfileViewingPolicyAdapter:
    """The adapter exposes the caller's viewing policy to the Watch Progress BC."""
    ...
    async def test_should_return_the_policy_the_profile_derives(
        ...
        assert policy == profile.viewing_policy()
    ...
    async def test_should_return_deny_all_policy_for_unknown_profile_id(
        ...
        # A session pointing at a deleted profile must see no progress
        # and record none, rather than fall back to an open policy.
        adapter = _make_adapter(session_factory)
